...
<template>
  <div class="min-h-screen bg-void pb-24 relative selection:(bg-primary-500/30 text-white) overflow-x-hidden">
    <!-- Atmospheric Accents -->
    <div class="fixed top-0 right-0 w-[500px] h-[500px] bg-primary-500/5 blur-[150px] rounded-full pointer-events-none"></div>
    <div class="fixed bottom-0 left-0 w-[600px] h-[600px] bg-indigo-500/5 blur-[180px] rounded-full pointer-events-none"></div>

    <div class="max-w-[1800px] mx-auto px-4 md:px-10 lg:px-16 pt-12 relative z-10">
      <!-- Header Section -->
      ...
      <div v-if="user && subscriptions?.length" v-motion-fade-in class="mb-16">
        ...
        <div class="flex items-center gap-8 overflow-x-auto pb-6 scrollbar-none">
          <NuxtLink v-for="sub in subscriptions" :key="sub.creator_id" :to="`/profile/${sub.creator_id}`" 
      ...
      <!-- Pending State -->
      <div v-else-if="pending" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-8 gap-y-16">
          <VideoSkeleton v-for="n in 10" :key="n" />
      </div>

      <!-- Empty State -->
      <div v-else-if="subscribedVideos?.length === 0" v-motion-fade-in class="flex flex-col items-center justify-center py-32 text-center glass-card border-white/5 rounded-[3rem] shadow-2xl">
      ...
      <!-- Video Grid -->
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-8 gap-y-16">
          <VideoCard
              ...
              :enter="{ opacity: 1, y: 0, transition: { delay: idx * 100, duration: 800 } }"
          />
      </div>
    </div>
  </div>
