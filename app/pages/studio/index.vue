...
<template>
  ...
  >
    ...
    <div class="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      <div
        class="absolute -top-[10%] -left-[5%] w-[40%] h-[40%] bg-primary-500/5 blur-[120px] rounded-full animate-ambient"
        style="animation-delay: 0s"
      ></div>
      <div
        class="absolute bottom-[20%] -right-[5%] w-[30%] h-[30%] bg-primary-500/3 blur-[100px] rounded-full animate-ambient"
    ...
    >
      ...
      >
        <div
          class="glass-card border-primary-500/20 p-10 md:p-12 max-w-md w-full space-y-8 text-center shadow-[0_0_50px_-12px_rgba(239,68,68,0.2)] relative z-10"
        >
          <div
            class="w-16 h-16 mx-auto rounded-2xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center"
          >
            <Icon
          ...
          <div class="space-y-3">
            <h2
              class="text-2xl font-black text-white uppercase tracking-tight italic"
            >
          ...
          <NuxtLink
            to="/auth/login"
            class="btn-primary w-full inline-flex rounded-xl py-4 font-black uppercase tracking-widest text-xs"
          >
    ...
    >
      <!-- Header -->
      <div
        class="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8"
      >
        <div class="space-y-2">
          <div class="flex items-center gap-3">
            <div class="h-1 w-8 bg-primary-500"></div>
            <span
              class="text-[10px] font-black text-primary-500 uppercase tracking-[0.3em]"
              >{{ $t("studio.command_center") }}</span
            >
          </div>
          <h1
            class="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter italic"
          >
        ...
        <NuxtLink
          to="/studio/upload"
          class="btn-primary flex items-center gap-3 rounded-xl px-8 py-4 font-black uppercase tracking-widest text-xs shadow-xl transition-all"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div
            class="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-4 group-hover:text-primary-500 transition-colors"
          >
            {{ $t(`studio.total_${key}`) }}
          </div>
          <div class="flex items-end justify-between">
            ...
            <div
              class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white/20 group-hover:bg-primary-500/10 group-hover:text-primary-500 transition-all"
            >
              <Icon
                :name="
                  key === 'views'
      ...
      <!-- Content Table -->
      <div :delay="300" class="space-y-6">
        <div class="flex items-center justify-between">
          <h2
            class="text-xl font-black text-white uppercase tracking-tight italic"
          >
        ...
        >
          <div
            class="w-24 h-24 rounded-[2rem] bg-white/[0.02] border border-white/5 flex items-center justify-center mb-2"
          >
            <Icon
              name="ph:video-camera-slash-duotone"
              class="text-4xl text-white/10"
            />
          </div>
          <div class="space-y-2">
            <p
              class="text-white/60 text-base font-bold uppercase tracking-tight"
            >
              {{ $t("studio.no_records") }}
            </p>
            <p class="text-white/30 text-xs uppercase tracking-widest italic">
              {{ $t("studio.no_records_hint") }}
            </p>
          </div>
          <NuxtLink
            to="/studio/upload"
            class="btn-primary mt-4 rounded-xl px-10 py-4 font-black uppercase tracking-widest text-xs"
          >
            {{ $t("studio.upload") }}
          </NuxtLink>
        </div>
        ...
        <div
          v-else
          class="glass-card overflow-hidden border-white/5 shadow-2xl"
        >
          <!-- Table Header -->
          <div
            class="hidden sm:grid grid-cols-[1fr_120px_140px_100px] gap-6 px-8 py-5 border-b border-white/5 text-[10px] font-black text-white/30 uppercase tracking-[0.2em]"
          >
          ...
          <div class="divide-y divide-white/5">
            ...
            >
              ...
              <div class="flex gap-6 min-w-0 w-full">
                <div
                  class="w-40 aspect-video rounded-2xl overflow-hidden bg-void-900 border border-white/10 shrink-0 relative shadow-xl group-hover:border-primary-500/30 transition-colors"
                >
                  <img
                ...
                <div class="flex-1 min-w-0 space-y-2 py-1">
                  <h3
                    class="font-bold text-base text-white truncate group-hover:text-primary-500 transition-colors tracking-tight"
                  >
                    {{ video.title }}
                  </h3>
                  <p
                    class="text-xs text-white/30 line-clamp-2 leading-relaxed font-medium"
                  >
                  ...
                  <!-- Mobile only meta -->
                  <div
                    class="flex sm:hidden items-center gap-3 text-[10px] font-black text-white/20 mt-3 uppercase tracking-widest"
                  >
                    <span>{{
                      video.created_at
              ...
              <div class="hidden sm:flex items-center justify-end">
                <span
                  class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary-500/10 text-primary-500 text-[10px] font-black uppercase tracking-widest border border-primary-500/20"
                >
                  <div
                    class="w-1.5 h-1.5 rounded-full bg-primary-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"
                  ></div>
                  {{ $t("studio.visibility_public") }}
                </span>
              </div>
              <div
                class="hidden sm:block text-[11px] font-bold text-white/40 text-right uppercase tracking-tighter"
              >
                {{
                  video.created_at
                    ? new Date(video.created_at).toLocaleDateString(locale, {
              ...
              <div
                class="hidden sm:block text-base font-black text-white text-right tabular-nums tracking-tighter"
              >
                {{ (video.view_count || 0).toLocaleString() }}
              </div>
              ...
              <!-- Hover Actions -->
              <div
                class="flex sm:absolute sm:right-8 sm:top-1/2 sm:-translate-y-1/2 items-center gap-3 sm:opacity-0 group-hover:opacity-100 transition-all duration-300 sm:translate-x-4 group-hover:translate-x-0 bg-void/90 sm:backdrop-blur-xl sm:pl-6 py-2 rounded-2xl mt-4 sm:mt-0 z-20"
              >
                <button
    ...
    >
      ...
      >
        ...
        <div
          class="glass-card border-white/10 w-full max-w-xl shadow-[0_0_100px_-20px_rgba(0,0,0,0.8)] relative z-10 flex flex-col max-h-[90vh] overflow-hidden"
        >
          <div
            class="flex items-center justify-between p-8 border-b border-white/5 bg-white/[0.01]"
          >
            <div class="space-y-1">
              <span
                class="text-[10px] font-black text-primary-500 uppercase tracking-[0.3em]"
            ...
            <button
              @click="closeEditModal"
              class="w-10 h-10 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all"
            >
          ...
          <div class="p-8 space-y-8 overflow-y-auto custom-scrollbar">
            ...
            <div class="space-y-3">
              ...
              >
              <div
                class="flex items-end gap-6 p-4 rounded-3xl bg-white/[0.02] border border-white/5 group"
              >
                <div
                  class="w-48 aspect-video rounded-2xl overflow-hidden bg-void-900 border border-white/10 shrink-0 shadow-2xl relative"
                >
                  <img
                  ...
                  <div
                    ...
                    class="w-full h-full flex items-center justify-center text-[10px] text-primary-500 font-black uppercase tracking-widest bg-primary-500/10 italic"
                  >
                    {{ $t("studio.updated") }}
                  </div>
                  <div
                ...
                <div class="flex-1 space-y-3">
                  ...
                  <button
                    ...
                    class="w-full px-6 py-3 glass-button rounded-xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl"
                  >
                    {{ $t("studio.change_image") }}
                  </button>
                  <p
                    class="text-[9px] text-white/20 font-bold uppercase text-center"
                    v-if="editForm.thumbnailFile"
                  >
                    {{ editForm.thumbnailFile.name }}
                  </p>
                  <p
                    class="text-[9px] text-white/20 font-bold uppercase text-center italic"
                    v-else
                  >
            ...
            <div class="space-y-3">
              ...
              <input
                ...
                type="text"
                class="glass-input w-full rounded-2xl py-4 px-6 text-sm font-medium"
            ...
            <div class="space-y-3">
              ...
              <textarea
                ...
                class="glass-input w-full min-h-[150px] resize-none rounded-3xl py-5 px-6 text-sm font-medium"
            ...
            <!-- 18+ Toggle -->
            <div
              class="flex items-center justify-between p-6 rounded-3xl border border-white/5 bg-white/[0.01] hover:border-primary-500/20 transition-all duration-500"
            >
              <div class="space-y-1">
                <div
                  class="text-sm font-bold text-white uppercase tracking-tight italic"
                >
                  {{ $t("studio.age_restriction") }}
                </div>
                <div class="text-[11px] text-white/30 font-medium">
              ...
              >
                <div
                  ...
                  :class="
                    editForm.is_18_plus ? 'translate-x-7' : 'translate-x-0'
                  "
                ></div>
              </button>
            </div>
          ...
          <div
            class="p-8 border-t border-white/5 flex justify-end gap-4 bg-white/[0.01]"
          >
            <button
              @click="closeEditModal"
              class="px-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white hover:bg-white/5 transition-all"
            >
              {{ $t("studio.cancel") }}
            </button>
            <button
              ...
              class="btn-primary rounded-xl px-10 py-4 text-[10px] font-black uppercase tracking-widest shadow-2xl disabled:opacity-50 transition-all"
            >
