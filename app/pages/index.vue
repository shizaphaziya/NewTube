...
<template>
  <div
    class="min-h-screen relative selection:bg-primary-500/30 selection:text-white"
  >
    <!-- Category & Sort Bar -->
    <div
    ...
    >
      ...
      <div class="flex items-center shrink-0 relative group/dropdown">
        <div
          class="flex items-center gap-3 bg-theme-surface border border-theme-border/80 rounded-xl px-5 py-2.5 text-[12px] font-700 text-theme-text-muted transition-all cursor-pointer hover:border-primary-500/40 hover:text-theme-text hover:shadow-lg hover:shadow-primary-500/5 group-hover/dropdown:border-primary-500/40 dropdown:text-theme-text"
        >
          <div
            :class="sortOptions.find((o) => o.key === selectedSort)?.icon"
            class="text-xl text-primary-500 transition-transform group-hover/dropdown:scale-110"
          ></div>
          <span class="translate-y-px tracking-tight">{{
            sortOptions.find((o) => o.key === selectedSort)?.label
          }}</span>
          <Icon
            name="ph:caret-down"
            class="text-sm transition-transform duration-500 group-hover/dropdown:rotate-180 opacity-50"
          />
        </div>

        <!-- Dropdown Menu -->
        <div
          class="absolute top-full left-0 mt-3 w-64 glass-panel rounded-2xl p-2 opacity-0 invisible -translate-y-2 scale-95 group-hover/dropdown:opacity-100 dropdown:visible dropdown:translate-y-0 dropdown:scale-100 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-50 overflow-hidden shadow-2xl"
        >
          <div
          ...
          <button
            ...
            :class="
              selectedSort === opt.key
                ...
                : 'text-theme-text-muted hover:bg-theme-border/40 hover:text-theme-text'
            "
          >
            <div class="flex items-center gap-3">
              <div
                :class="[
                  opt.icon,
                  selectedSort === opt.key ? 'text-white' : 'text-primary-500',
                ]"
            ...
            <Icon
              name="ph:arrow-right"
              class="text-lg opacity-0 -translate-x-2 group-hover/item:opacity-100 item:translate-x-0 transition-all text-primary-500"
              v-else
            />
          </button>
        </div>
        ...
        <button
          ...
          :class="
            cat.key === 'all'
              ...
              : 'bg-theme-surface border-theme-border/80 text-theme-text-muted hover:border-primary-500/30 hover:text-theme-text hover:bg-theme-surface-raised'
          "
        >
          {{ cat.label }}
        </button>
    ...
    <section class="pb-32 px-4">
      ...
      >
        <div class="flex items-center gap-5">
          <div
            class="h-8 w-1.5 bg-primary-500 rounded-full shadow-[0_0_15px_rgba(239,68,68,0.4)]"
          ></div>
          <h2
            class="text-2xl md:text-3xl font-800 text-theme-text tracking-tight uppercase"
          >
            {{ $t("home.latest") }}
          </h2>
        </div>
        ...
        <div
          class="flex items-center gap-2.5 text-[11px] font-700 text-theme-text-muted hover:text-primary-500 transition-all cursor-pointer group uppercase tracking-widest"
        >
          <span>{{ $t("home.view_all") }}</span>
          <Icon
            name="ph:caret-right-bold"
            class="text-lg group-hover:translate-x-1 transition-transform duration-300 text-primary-500"
          />
        </div>
      </div>
