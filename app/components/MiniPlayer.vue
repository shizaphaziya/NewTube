...
<template>
  ...
  >
    <div class="relative aspect-video bg-stone-900 group">
      ...
      <button
        @click="close"
        class="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"
      >
        <div class="i-ph-x-bold text-xs"></div>
      </button>
      <NuxtLink
        :to="`/watch/${appStore.activePipVideo.id}`"
        class="absolute top-2 left-2 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"
      >
