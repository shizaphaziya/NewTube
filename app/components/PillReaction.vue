...
watch(
  () => props.initialDislikes,
  (newVal) => {
    localDislikes.value = newVal;
  },
);
...
<template>
  <div
    class="flex items-center rounded-md bg-white/[0.03] border border-white/[0.08] p-1 shadow-inner overflow-hidden transition-colors hover:bg-white/[0.05]"
  >
    <!-- Like Zone -->
    <button
      @click="toggleLike"
    ...
    >
      <Icon
        ...
        class="[ currentState === 'like' ? '' : 'i-ph-thumbs-up-bold', ]"
    ...
    >
      <Icon
        ...
        class="[ currentState === 'dislike' ? '' : 'i-ph-thumbs-down-bold', ]"
